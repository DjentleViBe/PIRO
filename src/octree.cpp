...
int Piro::mesh_operations::octree::findind(std::vector<std::vector<int>>& pm, int match, int level){
    // Piro::logger::info("Size : " ,pm.size());
    for(int k = 0; k < pm.size(); k++){
        // Piro::logger::info(pm[k][1], pm[k][0]);
        if(pm[k][8] == match && pm[k][0] == level - 1){
            return k;
        }
    }
...
std::vector<float> Piro::mesh_operations::octree::get_ijk(std::vector<int> index, std::vector<float>& coord, std::vector<std::vector<int>>& pm){
    ...
    if (level <= 1){
        ...
        float dx = 1 / pow(2, (level - 1));
        float dy = 1 / pow(2, (level - 1));
        float dz = 1 / pow(2, (level - 1));
        
        i_val[0] = parent_coord[0] + (-pm[match][2] + index[2] / 2) * dx;
        i_val[1] = parent_coord[1] + (-pm[match][4] + index[4] / 2) * dy;
        i_val[2] = parent_coord[2] + (-pm[match][6] + index[6] / 2) * dz;
        for(int i = 0; i < std::min(coord.size(), i_val.size()); i++){
