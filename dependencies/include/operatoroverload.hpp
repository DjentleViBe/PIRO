...
class CLBuffer{
    public:
        cl_mem buffer;
        friend CLBuffer operator*(const CLBuffer& partA, const CLBuffer& partB){
            cl_int err;
            ...
            err |= clSetKernelArg(kernel_multiplyVec, 0, sizeof(cl_mem), &partA.buffer);
            err |= clSetKernelArg(kernel_multiplyVec, 1, sizeof(cl_mem), &partB.buffer);
            err |= clSetKernelArg(kernel_multiplyVec, 2, sizeof(cl_mem), &partC);
